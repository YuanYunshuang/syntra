...
def load_sam2_img_encoder_to_syntra(
    state_dict: Dict,
    model: nn.Module,
    strict: bool = True,
):  
    img_encoder_state = {k: v for k, v in state_dict.items() if k.startswith("image_encoder.")}
    missing_keys, unexpected_keys = model.load_state_dict(img_encoder_state, strict=False)
